    ...
    [
      _c(
        ...
        [
          _c(
            "template",
            { staticStyle: { "z-index": "10000" }, slot: "suffix" },
            [
              _vm.clearable && _vm.inputHover && _vm.currentLabels.length
                ...
                : _c("i", {
                    ...
                    staticStyle: { position: "relative", "z-index": "100000" },
...
/* harmony default export */ var mainvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    AddhandleInputChange: function AddhandleInputChange(value) {
      var _this5 = this;

      if (value === '') {
        this.menuVisible = false;return;
      };
      if (!this.menuVisible) this.menuVisible = true;
      this.$nextTick(function () {
        _this5.handleInputChange(value);
      });
    },
    handleInputChange: function handleInputChange(value) {
      ...
      var filteredFlatOptions = flatOptions.filter(function (optionsStack) {
        return optionsStack.some(function (option) {
          return new RegExp(Object(util_["escapeRegexpString"])(value), 'i').test(option[_this6.labelKey]);
        });
      });
        ...
        filteredFlatOptions = filteredFlatOptions.map(function (optionStack) {
          return {
            ...
            value: optionStack.map(function (item) {
              return item[_this6.valueKey];
            }),
            label: _this6.renderFilteredOptionLabel(value, optionStack),
            disabled: optionStack.some(function (item) {
              return item[_this6.disabledKey];
            })
          };
        });
      } else {
    ...
    renderFilteredOptionLabel: function renderFilteredOptionLabel(inputValue, optionsStack) {
      ...
      return optionsStack.map(function (option, index) {
        var label = option[_this7.labelKey];
        var keywordIndex = label.toLowerCase().indexOf(inputValue.toLowerCase());
        var labelPart = label.slice(keywordIndex, inputValue.length + keywordIndex);
        var node = keywordIndex > -1 ? _this7.highlightKeyword(label, labelPart) : label;
        return index === 0 ? node : [' ' + _this7.separator + ' ', node];
      });
    },
    highlightKeyword: function highlightKeyword(label, keyword) {
      ...
      return label.split(keyword).map(function (node, index) {
        return index === 0 ? node : [h('span', { class: { 'el-cascader-menu__item__keyword': true } }, [_this8._v(keyword)]), node];
      });
    },
    flattenOptions: function flattenOptions(options) {
      ...
      options.forEach(function (option) {
        ...
        if (!option[_this9.childrenKey]) {
          flatOptions.push(optionsStack);
        } else {
          if (_this9.changeOnSelect) {
            flatOptions.push(optionsStack);
          }
          flatOptions = flatOptions.concat(_this9.flattenOptions(option[_this9.childrenKey], optionsStack));
        }
      });
      return flatOptions;
    },
  ...
  created: function created() {
    ...
    this.debouncedInputChange = debounce_default()(this.debounce, function (value) {
      var before = _this10.beforeFilter(value);

      if (before && before.then) {
        _this10.menu.options = [{
          __IS__FLAT__OPTIONS: true,
          label: _this10.t('el.cascader.loading'),
          value: '',
          disabled: true
        }];
        before.then(function () {
          _this10.$nextTick(function () {
            _this10.AddhandleInputChange(value);
          });
        });
